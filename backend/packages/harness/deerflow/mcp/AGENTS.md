...
- **Child runtime boundary**: the current authenticated-public-web Provider is started lazily behind `douyin_public_evidence` and reused through `MCPSessionPool`; only seven allowlisted read-only calls can cross it. The gateway validates Child input/output Schemas and a 64 KB result ceiling. Cookie values, raw pages, temporary media URLs, and child stderr never enter the model response. The official SSE bridge remains internal and cannot disclose a Child until a live tool contract is reviewed; a successful initialize with zero tools remains unavailable.
- **Douyin readiness evidence**: `community/douyin_openapi/readiness.py` merges the immutable 119-row inventory with a credential-free live observation. Keep catalog presence, reviewed adapter, locally declared Scope, provider decision, and exact live receipt separate. A successful token exchange is auth infrastructure, not a live business capability. Regenerate the committed report through `scripts/render_douyin_openapi_readiness.py`; never write credential values or a tokenized SSE URL into its observation.
