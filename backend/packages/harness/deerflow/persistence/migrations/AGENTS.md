...
- `migrations/versions/0016_mcp_task_submission_policy.py` — adds `mcp_tasks.submission_policy` and `submission_started_at`; existing rows remain `idempotent_retry`, while paid providers without an audited idempotency key use `at_most_once` and terminal `submission_unknown` recovery instead of automatic resubmission. Downgrade terminalizes every pending `at_most_once` intent before removing the policy columns so an 0015 worker cannot reclaim it.
- `migrations/versions/0017_account_launch_plan_unique.py` — adds the partial unique index `uq_incubation_artifacts_account_launch_plan_version`, the cross-Gateway arbiter allowing at most one `account_launch_plan` artifact per owner/project/logical-account/version. Upgrade fails closed without deleting immutable artifacts when an existing scope is duplicated; downgrade removes only the index.
- `persistence/bootstrap.py` — `bootstrap_schema(engine, backend=...)`, the three-branch decision + locking
- `migrations/versions/0014_incubation_approval_grants.py` — exact cloud-processing and fee grants with one-task binding
- Tests: `tests/test_persistence_bootstrap.py` (branches), `tests/test_persistence_bootstrap_concurrency.py` (concurrency), `tests/test_persistence_bootstrap_regression.py` (issue #3682), `tests/test_persistence_migrations_env.py` (filter), `tests/blocking_io/test_persistence_bootstrap.py` (asyncio.to_thread anchor), `tests/test_migration_0004_run_ownership_dedupe.py` + `tests/test_migration_0007_scheduled_run_active_dedupe.py` (dedupe-before-unique-index pre-steps), and `tests/test_migration_0017_account_launch_plan_unique.py` (fail-closed duplicate upgrade plus non-destructive downgrade)
