...
断言只允许做一次删除或泛化修复；仍然越界就拒绝交付。有合格交付时，终端先显示本题采用的内容机会
依据，再显示一个地图内的“今日建议拍摄”；若已有与该地图精确匹配的账号策略，则另行展示其版本，
不会把候选地图冒充成已经采用的定位。
...
内部地图只在没有足够证据或交付合同失败时作为开放回执。基础稿之后可以单独生成本条
`FormatDecision` 与 `AdaptedDraft`；资源未知时形式保持暂定，并保留备选。编剧方法只在选中叙事形式
时按需调用。运行可以在表达稿后停止，ProductionPlan、素材任务、MediaKit 和发布属于独立下游能力，
默认关闭，不参与本轮孵化与表达验收。
