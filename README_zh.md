...
长期账号策略由独立的 `develop_account_strategy` 生成。它要求先选择项目，读取候选地图、项目事实、
正式对标快照和正式受众观察，再形成可版本化的 `IncubationJudgment`，分别保存定位、受众假设、人设、
账号级表现方向和变现假设。相同输入复用原版本；事实、证据或候选地图变化时生成带上一版父级和修改
原因的新版本。`BenchmarkSnapshot` 只提供观察，不能自行下定位结论。选题链只读取与当前候选地图精确
匹配的现有策略，不会为了完成一条内容临时创造或修改账号定位。
