...
长期账号策略由独立的 `develop_account_strategy` 生成。首次起号先创建
`MarketingSubjectSnapshot`：普通业务只读用户与项目事实；“你自己”明确指向当前 Agent 时，系统读取
服务端版本化 `HostProductProfile` 中的产品事实、能力、已接受证据和限制，不把代词交给语义模块猜，
也不让模型自行扩充产品能力。
普通业务的主体必须是用户原话中的连续片段。系统先用本地精确词条检查该表达；近期新词、专名、缩写或
未收录行业词才会触发一次有界公开网页搜索。最多三条摘要以独立 `term_evidence` 同时供受众和语义读取，
不会混入对标账号或选题资料，也无权选择内容根。搜索失败保留未知；Agent 自营主体继续只信任本地产品档案。
随后 `AccountAudienceDecision` 在内容根之前分开付款或签约者、决策者、使用或受益者、业务真正需要
影响的人和愿意长期观看的内容受众。交易关系已明确时直接解析；批发/零售、B 端/C 端等分歧会实质
改变账号方向时，系统只给二至三条受众路线并等待用户选择，不提前运行内容地图、对标和账号策略。
没有正式证据时，年龄、收入、地域等人口统计信息保持未知。
受众确定后，工具读取候选地图、项目事实、正式对标快照和正式受众观察，再形成可版本化的
`IncubationJudgment`，分别保存定位、受众假设、人设、账号级表现方向和变现假设。相同输入复用原版本；
事实、证据或候选地图变化时生成带上一版父级和修改原因的新版本。`BenchmarkSnapshot` 只提供观察，
不能自行下定位结论。选题链只读取与当前候选地图精确匹配的现有策略，不会为了完成一条内容临时创造
或修改账号定位。
每条新路线还会单独保存业务意图：用户在已声明业务中的角色、曝光之后账号要完成的业务任务、需要影响
...
爆款原因或可复制公式。合同已实现；抖音账号链接的生产采集器仍需单独接入和真实账号验收。
...
项目台账。它仍不是稳定账号身份，必须再通过官方账号路径取得作者一致多作品和覆盖回执，
才能封存为 `BenchmarkSnapshot`。星图和百应不再是公开内容样本的前置依赖，只在官方搜索缺少所需字段时补充。
