...
1. **检查依赖环境**：
   ...
   本地 `make check`、`make install`、`make dev` 和 `make start` 都通过共享运行器调用 pnpm。运行器在 `frontend/` 中执行，并强制使用 `frontend/package.json` 锁定的 pnpm 版本，不会被本机全局 pnpm 的版本忽略设置绕过。
...
内容机会探索可以调用 `explore_content_world`。当前主链按认知依赖顺序运行：业务语义、
可选词义证据、语义家族、可空的共同世界、独立反事实复核、一次内容根裁决和冻结根地图，随后才展开
地图方向搜索、具体人物与事件召回、证据阅读和创意收敛。这些受控工作者不是可写状态的自由子 Agent；
模型产生的中间语境标签也不会再被编译成候选硬门。场所、商品、消费动作和社交结果拼成一个长场景，
不自动成为更大的内容根；如果商品仍能在该场景之外展开人物、历史、事件、地域和作品，场景只是地图
中的一条路径。根裁决进一步区分本次阅读的具体 `content_entry` 与候选地图 `map_root`：入口只解释
商业表达如何走进内容世界并保存在阅读台账，候选地图、版本哈希、搜索和选题只绑定经复核的地图根，避免
靠近商品的用途或场景再次收缩内容机会。共同世界合成时只看独立意义成分及其语义家族，不提前接收
该成分在原复合词中的用途解释；独立复核者
随后再取得这层复合词关系，检查扩展后的世界能否双向解释用户表达。这样既不让近商品义提前占满注意力，
也不把同字联想直接当成成立的语义跃迁。内容地图现在是**候选内容机会**：它记录从哪个内容根可以
展开哪些人物、时间、地点、事件、种类、文化、作品和关系。它不拥有账号定位、受众、人设、账号级
表现形式或变现判断权，也不会让婚礼、购买、到店、宴请等一个具体场景篡位成整个内容世界。
候选地图拥有稳定版本，后续选题必须绑定该版本并从地图根起步；热点只能成为地图路径上的新证据。

可复用的行业孵化经验不再写进通用 Lead 提示词。匹配的 `incubate-*` Skill 只在需要时加载，可以提供
有界候选路径、可选的版本化默认内容根、局部分支边界和事实边界，但不能改写用户原话，也不能决定
账号定位、受众、人设、表现形式或变现。

默认 Lead 现在直接拥有账号孵化判断，不再要求每个“怎么起号”请求先调用固定路线工具。它会根据当前
不确定性选择直接回答、一个会改变路线的澄清、一个匹配行业 Skill、一次陌生词核实、语义分析、一次
内容地图或对标证据；这些都不是必经阶段。历史 `develop_account_strategy` 仍用于旧工件兼容和离线验收，
但已从默认 Lead 工具目录移除。

Lead 现在还使用一份所有模型共用的薄运营执行者合同：把明确请求当作当前任务，交付最小可用结果，不可逆外部动作仍须
明确审批。首轮账号方向先用用户事实形成临时判断，不再自动搜竞品；只有用户要求当前证据，或陌生词需要有界核实时才搜索。
交付前的短复核位于母提示最后，但系统不使用中间件偷改模型回答。真实 GLM 验收已解决首轮过度搜索，但仍可能偶发追加顾问式后续邀约，
因此“员工本体感”仍以真实会话验收，不因 Prompt 已存在就宣称通过。

普通业务继续只信任用户与项目事实；“你自己”明确指向当前 Agent 时，只读服务端版本化
`HostProductProfile`。近期新词、专名、缩写或歧义行业词可触发一次最多三条摘要的有界核实，结果只属于
`term_evidence`，不能决定受众、内容根或对标。付款者、决策者、受益者、业务目标人群和内容受众仍保持
区分，但只有它们的差异会实质改变账号方向时，Lead 才问一个选择题；未提供的人口统计、案例、资源、渠道
和权威保持未知。宽泛起号问题默认只回答方向，不擅自补发布日历、频率、时段、比例、投流或 7/30 天计划。

值得跨轮复用的判断现在可以封存为 `AccountDirectionProposal`；只有用户对精确提案选项的真实确认才会形成
追加式 `AccountDirectionVersion`。7/30 天 `AccountLaunchPlan` 仍是可选继续，只能同时消费当前方向和一张
与其精确关联的候选地图，并返回可复核的计划提案编号。计划题眼不是脚本或自动制作任务，仍须进入取证、
`TopicBrief -> MessagePlan -> BaseDraft`。这条继续只接收成对的“当前已确认计划工件 ID + 计划内题眼 ID”；
Host 从当前逻辑账号台账解引用精确计划、地图和账号决策父级，忽略模型自由填写的 `topic_seed`。
`content_reading` 保存计划 ID、哈希和题眼 ID，它与 `TopicBrief` 都直接绑定该精确计划父级。

当用户明确进入正式版本化路线时，系统仍可保存业务意图：用户在已声明业务中的角色、曝光之后账号要完成的业务任务、需要影响
的人、对方需求、希望促成的行动和用户明示的市场范围。它与愿意持续观看的内容受众、以及真人口播、
素材叙事等表现形式相互独立；候选路线不能只换拍法，变现也不能凭空新增课程、SaaS、咨询或其他生意。
旧台账可以没有这个切面，新生成的账号路线必须提供。
...
代码再将开头、内容推进和收束组合为形式无关的基础文案。封存前，高信号的证据外专名、数字和绝对
断言只允许做一次删除或泛化修复；仍然越界就拒绝交付。有合格交付时，终端只显示一个地图内的
“今日建议拍摄”、一条内容路线和一份基础稿；完整候选地图、账号策略、重复内容节拍和制作建议留在
工件台账。账号立场精确绑定项目主体原话，不能据此虚构用户做了多久、见过或经手过什么，也不能假定
已有门店素材、客户案例、库存、团队或出镜人员。详细限制保存在台账，聊天只显示数量状态和引用。
用户确认账号路线后再要具体选题时，工具会从台账重建该路线精确绑定的原始阅读记录和内容寻址候选地图，
不再把“稿子”、“今天拍什么”等交付用语重新识别成内容根。真正引入新主体时，新主体仍必须是当轮用户原话中的连续片段。
研究召回数量只是成本上限，不是交付配额；供应商结构兼容也只解包与目标字段容器类型一致的明确 JSON，普通文本仍会拒绝。
内部地图只在没有足够证据或交付合同失败时作为开放回执。普通选题请求默认在 `draft_version` 停止；
只有用户明确要求表现形式、适配成稿或制作时，才继续生成本条 `FormatDecision`、`AdaptedDraft` 或
`ProductionPlan`。每项资源匹配必须引用正式用户素材父工件，未知资源继续保持未知。编剧方法只在选中
叙事形式时按需调用；素材任务、MediaKit 和发布属于独立下游能力。

新增公开 `marketing-video-production` Skill 作为“精确封存的 `AdaptedDraft` 已存在，且用户明确要求继续
制作视频”之后的唯一下游指南。当前它能审阅已经形成的对标视频证据、准备与计划绑定的素材/Provider 合同，
以及路由已验收的本地裁剪纵切；不会自动执行 MediaKit ASR/OCR/场景分析或 Ark 生图/生视频。
它复用第六版既有 `ProductionPlan` 和事实台账，不另建镜头状态机；山音
提炼出的镜头方法只作为方案背后的可选知识。对标视频的机器观察、编辑解释、可迁移结构、身份/品牌装潢
禁迁项和未知保持分层。Ark 单镜头请求合同与最多提交一次的持久任务目前仍是未注册安全底座，只有 Gateway
审批入口和一次带精确用户授权的全新真实回执通过后才可启用；代码存在不等于已经产生付费调用。

第六版业务台账现在还提供平台无关的 `BenchmarkSnapshot`，用于封存用户指定的第三方
对标账号观察。它将一个稳定账号身份与最多 24 条作者一致的作品绑定，显式保留
返回数、排除数和采样限制，并只给 Lead 固定字节预算的投影。它不是粉丝画像、账号定位结论、
爆款原因或可复制公式。合同和认证浏览器账号链接采集器已经实现，并通过一条指定抖音账号的真实切片；
这只证明该连接器可继续受控使用，不代表所有账号或页面版本都已支持，官方账号链路与更广真实回归仍待完成。

抖音公开视频搜索会按当前应用真实获批的 Scope 选择现行 v1 合同或兼容历史 v2 合同。
同一有界适配器可按本地目的生成选题证据，或生成
`benchmark_account_candidate` 用于发现对标候选；该目的参数不会发送给抖音。搜索返回的作者
显示名现在可以在有界搜索页中精确筛选，按公开作品 ID 去重，并将排除和翻页覆盖一起封存到
项目台账。它仍不是稳定账号身份。选中候选后，可用高层 `collect_douyin_benchmark_account` 从用户明确给出的
账号链接执行认证浏览器采集，只保留稳定作者一致的作品并封存 `BenchmarkSnapshot`；官方账号路径仍是后续
优先链路。星图和百应不再是公开内容样本的前置依赖，只在官方搜索缺少所需字段时补充。

Lead 现在可以通过单个高层 `collect_douyin_benchmark_candidate` 工具获取这份样本，
不会看到底层领域工具、密钥、所有者身份、分页游标或搜索回执。未选孵化项目时，
结果仍可作为只读证据使用；有服务端注入的项目 ID 时，系统先校验认证用户的所有权，
再将完整快照归属到当前会话和运行。台账写入失败会单独标记，不会丢掉已采集证据。当前应用 Scope 明确
不可用时，候选工具可以退回本地可见、已认证的 Playwright 会话；只有真实搜索页中白名单
`general/search` JSON 响应才算证据，建议词、热榜、DOM、HTML 和截图均不算。候选发现、指定账号采集和
单视频拆解是三个不同证据边界。
...
**不含正文的上下文账单**：每次 Lead 真实调用模型都会追加一条 `context:manifest` 运行事件，记录打包身份版本、消息和隐藏上下文大小、可见工具与 Schema 哈希、当前 Skill 激活来源，以及供应商返回的 Token 用量。它不保存 Prompt 正文、工具参数、Skill 路径、凭据或供应商报错原文，也不会改写模型请求。

**有来源的用户档案**：Lead 可以显式记住、纠正或忘记少量稳定的跨项目用户事实与协作偏好。每次修改必须引用当前最后一条可见用户消息中的原文，按用户保存为不可变修订，并在 2,400 字节预算内投影。项目/账号事实、模型推断、临时要求和操作授权不会进入档案；子 Agent 不能修改它。`context:manifest` 只记录版本、哈希和条目数。
