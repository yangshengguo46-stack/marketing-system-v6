...
宽泛的起号与长期内容请求可以调用 `explore_content_world`。它先并发运行“业务语义”和
“词义世界”两个受控工作者，再由代码校验词法主词的相同或嵌套范围、词义路径和构成语境，交给唯一总脑冻结候选，
随后展开内容地图、根后证据研究与创意收敛。两个工作者互相看不到对方答案，也不是可写状态的自由
子 Agent；主链由原来的六次串行模型调用缩为四次调用、三个关键阶段。内容地图是账号级
