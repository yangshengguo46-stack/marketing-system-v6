...
def _generate_video_minimax(
    ...
    access_value = os.getenv("MINIMAX_API_KEY")
    if not access_value:
        return "MINIMAX_API_KEY is not set"
    host = _minimax_host()
    auth = f"Bearer {access_value}"
    body = {
...
def download(url: str, output_file: str) -> None:
    access_value = os.getenv("GEMINI_API_KEY")
    if not access_value:
        raise ValueError("GEMINI_API_KEY is not set")
    response = requests.get(url, headers={"x-goog-api-key": access_value}, timeout=300)
    response.raise_for_status()
    _ensure_output_dir(output_file)
    with open(output_file, "wb") as f:
        f.write(response.content)
...
def _generate_video_gemini(
    ...
    response = requests.post(
        ...
        headers={"x-goog-api-key": access_value, "Content-Type": "application/json"},
        json=request_json,
        timeout=60,
    )
    response.raise_for_status()
    ...
    while True:
        response = requests.get(
            ...
            headers={"x-goog-api-key": access_value},
            timeout=30,
        )
        response.raise_for_status()
        data = response.json()
