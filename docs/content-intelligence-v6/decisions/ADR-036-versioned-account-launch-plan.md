...
双账号工件隔离、线程可信账号切换和旧数据迁移已通过，因此 `AccountLaunchPlan` 已注册为用户明确要求
才调用的可选 Lead 工具。它没有进入默认流程，TopicBrief 也不依赖计划确认。

本 ADR 仍保持 `proposed`：只读 Account Skill 投影重建与真实模型“确认路线 -> 计划 -> 选择一个题眼
-> TopicBrief”尚未验收。完成这些条件前，不授权自动调度、制作或发布。
