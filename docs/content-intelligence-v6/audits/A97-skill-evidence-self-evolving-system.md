...
第五版及旧版已有仓颉适配思路、金枪大叔、薛辉小清新、亲爱的安先生、文案三把刀等方法资料；第六版又
已有抖音 OpenAPI/MCP、MediaKit 和 HLLM 边界。它们能否组合成一套会随真实运营不断进步的系统，而
不是继续依靠人工改长提示词？
...
| 亲爱的安先生 | 收录在 `distill-screen-methods` 的个人 IP 创作者方法图谱，不是独立 Skill 目录 | 提出概念构建、并置/演绎/转换、时间语法、观众参与和“网感回到具体的人”等机制候选 | 不把电影方法原样压缩到短视频，不用抽象审美替代人物、场景和事实 |
...
金枪大叔、薛辉、亲爱的安先生和文案三把刀的方法卡只读取这个证据包，并提出
`MechanismHypothesis`，例如：

- 这条内容是否适合用“语言钉子 + 可见证据”表达；
- 是否适合把讲解改成一个有真实停止条件的反向测试；
- 是否应该把抽象观点构建成人物、规则、对照和正在发生的时间过程；
- 这一条的首要任务是停留、信任还是咨询；
- 同一主张在抖音上应改变哪个适配变量。
...
1. 将 `video-pattern-learning` 的五层证据语义薄迁入第六版证据编译边界；不迁移仓颉旧编译器、自动安装
   和固定样本数量规则。
2. 将四位创作者图谱拆成小型、版本化的方法卡，保留 `sourced_fact / cross_source_synthesis /
...
但截至本审计，它还不是已运行的自进化系统。仓颉适配仍是隔离资产，四套创作者方法仍是方法图谱，抖音
