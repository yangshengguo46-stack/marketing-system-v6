...
和未知；后者把定位、受众假设、人设、账号级表现形式与变现假设分开，并绑定精确 Brief、地图版本和
实际采用的对标/受众证据。孵化判断可以修订，但不能反向改写语义或内容地图。账号级表现形式也不替代
单条内容的 `FormatDecision`；变现路径不进入 TopicBrief 或基础文案。

孵化判断运行时只接收已封存父产物。完整对标和受众快照保留在台账，模型读取各合同的有界投影；
冻结地图也只投影版本、根、受众疆域、长期承诺、观察方法和防漂移边界。一次判断模型输入上限为
...
7. 根裁决器分别处理两个职责：`content_entry` 是从商业表达进入内容世界的具体语义入口；`content_root` 是账号长期占领的最大有效世界。通过独立复核的共同人类世界拥有长期领地的解释权，较窄入口不能因更靠近商品而把它覆盖。入口写入本次阅读台账，不进入地图模型或地图版本。
8. 地图专家只看冻结的长期 `content_root`，看不到 `content_entry`、商品、用户原话和销售目标。它在同一次调用中形成账号长期承诺、稳定观察方法、防漂移边界和内容疆域，不生成每日选题，也不选择表现形式。
...
13. 代码将已审议的开头、内容推进和收束组合成形式无关的 `BaseDraft`。普通起号请求默认先返回一条地图内的“今日建议拍摄”，再附长期定位依据；只有明确只问长期定位时才停在地图。可拍目标无选题或交付失败时明确说明失败，不把地图正文冒充成具体选题。
...
内容地图首先是账号定位：`content_entry` 只解释本次商业表达从哪里进入；`content_root` 决定长期讲什么，`editorial_promise` 决定观众反复获得什么，`recurring_lens` 决定账号怎样持续观察世界；维度与路径再回答“冻结根周围有哪些可研究的人、时间、地点、事件、种类、文化、作品和关系”。地图模型只接收 `content_root`，因此具体入口不能把长期世界重新收缩成商品用途或单一场景。它不包含一个名为“冲突”的固定轴，也不把座次、让步、观点差异、利益差异、情绪波动或一般关系张力升级成故事。

地图版本只由长期定位和内容疆域决定。`content_entry`、研究阶段后来发现的人物、作品、事件和证据都不会改变版本；每个 `TopicBrief` 必须携带同一版本 ID，并从长期内容根作为路径第一步。实时热点可以成为某条路径上的新事件，但不能凭热度重写内容根、关注承诺或观察方法。这正是“做 IP”和“每天做视频”的工程分界。
