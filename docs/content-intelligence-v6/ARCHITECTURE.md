...
1. 语义阅读只拆商业表达，不选内容方向。
2. 跨场景共同世界只看去修饰后的词法主词、参与活动和具体场景，提出共同动作、参与者关系与共同世界；它看不到完整商品名、材质和商品价值功能，也不选根。
3. 独立审查者只检查这个共同世界是否真由主词构成。换成大量无关对象后仍完整成立的普通使用、消费、制作、交易或聚会场景会被退回，不得升级为共同世界。
4. 代码从原始对象、词法主词、完整被服务对象、活动、功能和通过审查的共同世界装配候选，不再让另一个模型自由改名或拼接候选。候选保留 `served_object / subject_activity / subject_function_or_use` 等来源类型，具体场景仍是不可选择的分支。
5. 根裁决只看到对象角色与冻结候选的来源类型、名称。可选择索引只在 `root_candidate` 子集中解释，代码层不允许 `example_branch` 成为内容根或观众领地；普通制作、处理、食用或使用动作不能仅因词面更宽而压过完整对象。
6. 地图专家只看冻结的 `content_root`，看不到商品、用户原话和销售目标。
7. 冻结地图同时启动两路发现：模型按地图联想可核验专名，联网搜索则直接从实际地图方向发现真实材料；前者不能垄断查询预算，后者不等待前者给出名字。
8. 两路搜索回执合并去重后，先受控打开公开正文，再由证据阅读按来源强度选择路线，区分观察、关系、状态变化、解释、限制和未知，不负责立题。
9. 创意收敛只根据已经读出的证据形成 `TopicBrief`，证据不足时明确弃权；它不是额外的编剧 Agent。
10. 内容世界总编只把语义跃迁和冻结地图整理成 Markdown，不重新做起号、卖货或平台判断；确定性渲染器随后追加可选的证据选题，正文模型看不到也不能改写它。
...
首版不引入向量库、知识图谱数据库、行业关键词路由、固定问卷、固定候选数或完整 MCN 工作流。当前接通轻量网页搜索和受控公开正文读取；来源等级、浏览器证据和账号证据仍是后续模块，不能由搜索摘要冒充。
