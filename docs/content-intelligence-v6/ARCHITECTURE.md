...
黄金礼品失败基线证明，一次结构化调用同时要求语义、地图和起号交付会产生注意力竞争；后续真实回执又证明，让一个工作者同时生成候选、选择赢家和解释赢家，会形成自证式收敛。当前主链是两个首轮工作者并行、代码合流、单总脑裁决和冻结根地图，共四次模型调用、三个关键阶段。它们是同一次 Tool 运行内的受控结构化工作者，不是拥有工具和状态写权限的 DeerFlow `task` 子 Agent。

1. 语义阅读只拆商业表达、对象角色和修饰关系，不选内容方向。`source_object` 与 `lexical_head` 必须原样摘录用户表达；完整被服务对象使用最小通用名称，不继承可移除的地域、材质、价格或人群修饰。
2. 与它同时启动的词义世界专家只看逐字复制的 `subject_expression`，看不到业务语义结果、来源包或用户资源。它独立摘录 `lexical_head`，区分真语义成分与拆字，并且只允许社会关系、文化制度和人类关切进入跨域词义世界。普通使用、制作、消费、交易、聚会和工具动作留给业务语义，不在词义分支里强造共同世界。
3. 词义专家在同一结构化合同中给出语义家族、可空共同世界和替换反事实自审；它没有最终判断权。两位专家的词法范围完全相同时直接合流；若一个只是另一个在原商业对象中的连续嵌套范围，代码只保留两边都成立的真语义成分并重新绑定；完全无包含关系的主词撤下并记录未知。意义路径必须引用合流后仍成立的成分，自审也必须绑定同一个世界名称。
4. 可选本地词义证据在语义主词返回后并行读取，只在合流时过滤模型补造的词族表达，不作为答案注入首轮工作者。证据缺失或失败不阻断其他语义候选，也不恢复已被稠密向量实验否决的运行时。
5. 代码从原始对象、词法主词、语义核、完整被服务对象、完整参与活动、功能和通过审查的词义世界装配候选。语义专家用 `context_bound_worlds` 将完整对象、活动、事件、人群处境或关系世界绑定到构成修饰语；修饰语本身已是完整活动或事件时必须原样保留，不能扩大为上位类别或缩成一个环节。构成修饰语形成 `required_contexts`；只有显式保留这些上下文的候选才能被总脑选择。
6. 根裁决只看到对象角色与冻结候选的来源类型、名称和上下文绑定。可选索引只在 `root_candidate` 子集中解释，代码层不允许 `example_branch` 或丢失构成语境的候选成为内容根或观众领地。
...
12. 代码将已审议的开头、内容推进和收束组合成形式无关的 `BaseDraft`。普通起号请求默认先返回一条地图内的“今日建议拍摄”，再附长期定位依据；只有明确只问长期定位时才停在地图。可拍目标无选题或交付失败时明确说明失败，不把地图正文冒充成具体选题。
