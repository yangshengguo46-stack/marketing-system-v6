...
2026-08-19 在任何实现、证据采集和真实模型输出前冻结三组实验：只搜同行直接给方案；搜索同行、长期需求和
跨行业机制但不显式保留内容世界；使用相同广义证据并额外压缩一句长期主题与二至五个分支。七个全新案例
不设唯一标准内容根，覆盖宠物殡葬、老照片修复、儿童摄影、企业团建、工业防腐涂料、户外移动电源和夫妻
肺片调料包。

本机尚未配置抖音应用凭据，本轮只把无密钥公开搜索冻结为 `public_web_benchmark_discovery`，不冒充抖音
OpenAPI 或完整账号样本。三组共享证据、模型和预算，逐题盲审业务相关、长期连贯、差异迁移、可拍性、
证据诚实和成立边界。只有广义无地图组至少 `5/7` 可用且与薄内容世界组至少五题持平，才能判断显式内容根/
地图可以降级；否则若薄内容世界组在长期连贯或差异迁移上至少四题胜出，则采用“对标优先、薄内容世界后置”。
详见 `audits/A99-benchmark-first-incubation-preregistration.md`。
