...
2026-08-19 使用第五版 A39 已真实验收的“大能”账号包完成单账号、单行业、五调用烟测。输入由 17 条
作者一致作品、2 条代表视频产生的 16 个模式和 10 条伪名化可见互动组成；模型投影 `14,564` UTF-8
字节。两组都使用 GLM、关闭深度思考、温度 0，并各执行一次中间摘要和一次公共路线，另执行一次匿名复核；
总 Token 为 `26,145`。

真实对标成功提取了生活问题切入、个人经历串联物品、轻量实物展示、受众故事接续和成熟条件不可复制等
机制，证明账号拆解是有效证据模块。但纯对标组把“做腕表的”改写成非专家摸索者；薄内容世界组又擅自
假设维修、鉴定、估价能力并编造亡妻欧米茄、假劳力士和更换电容等客户案例。匿名裁判偏爱后者的完整性，
却没有把这些事实越界判为致命问题。

因此 ADR-028 保留语义理解和开放内容世界，把正式对标限定为只读机制证据：先保存用户事实与未知，再形成
候选内容世界，随后让对标支持、反驳或限制若干条件化路线，最后由用户确认。对标不能裁决内容根、补造用户
能力或把成熟账号现状倒推为冷启动模板。A101 唯一运行已消费，实验不接入现役 Agent。详见
...
2026-08-21 按 A120 删除行业 Skill 的四层营销硬控，并继续追出通用语义层在模型裁决后覆盖
map_root 的旧补丁。Profile 升为 schema v3：默认根改成可拒绝偏好，局部分支改成带 ID 和理由的
支持提示；路由说明与事实禁推项不进入选根上下文。自然语言分支分类、地图字段删除、路线全文子串拒绝和
两种裁决后覆盖均已删除，历史 Brief 的旧排除字段只读后丢弃。

独立收尾审查发现 Profile 的 do_not_assume 仍会无条件进入 Brief，且 Skill 名、版本与完整 Profile 哈希
仍出现在选根输入。最终实现进一步删除行业 Profile 的事实否定权，并将这些治理元数据移出模型上下文；
项目事实仍由通用 Brief 管理。

真实 GLM 最终将黄金礼品和新留出的企业客户答谢礼都收敛到“人与人之间的相处与人情世故”；明确经营
婚礼伴手礼时，原业务与局部地图路径保留，证明局部分支不再是禁词；但模型仍选择偏好总根，
说明软偏好强度尚未校准。企业答谢礼首轮在最终地图结构化输出处失败，做一次有记录的整链恢复后通过。
聚焦回归 136 passed，内容智能、孵化、Skill 与 Lead 接线宽回归 524 passed，礼赠 Skill 1.1.0
激活包已重签。成功链耗时 79.3 至 108.776 秒、总 Token 13262 至 17047，稳定率、外层恢复与性能
仍待后续单独验收。详见
audits/A121-soft-domain-skill-implementation-and-live-eval.md 与
decisions/ADR-035-soft-domain-skill-advice.md。

## A122 叙事骨架下游接线与真实故事回归

2026-08-21 追踪发现 `TopicBrief.narrative_frame` 在进入 `MessagePlan` 时被遗漏，导致上游即使
找到主角、目标、阻碍、行动、代价和结果，下游仍只能抽象说理。先写两条失败测试得到
`2 failed, 9 passed`；修复叙事投影和故事顺序后，交付聚焦测试为 `11 passed`。
内容合同、研究、工件谱系与 Lead 工具宽回归为 `181 passed`。

用户给定的小王/小张情景虽已进入故事主脊，模型仍补造了动机与接收者心理，严格记为部分失败。
改用新华社、云南人大网和国家保密局门户封存埃文·凯尔相册捐赠的完整行动链后，真实 GLM
按“收到相册 -> 面对不确定 -> 更正表述 -> 联系领馆 -> 无偿捐赠 -> 回赠与后续交往”先讲故事，
最后才点出礼物的分量来自对归属与关系的理解。该运行 `38.794s`、2 次调用、`8547 tokens`，
本地金丝雀通过。

完整自动研究初运行另行暴露三个问题：召回器把事件描述塞入人物字段，“埃文凯尔 / 埃文·凯尔”排版别名被当成换人，
以及行业 Skill 已有的公共礼仪与跨文化分支提示根本没有传到地图生成者。修复后仍保留真换实体的严格拒绝，行业分支也仍是可拒绝软候选。

从已冻结的人情世故根开始重跑地图、搜索、证据阅读、TopicBrief 与成稿，已得到“捐赠相册 -> 回赠国礼瓷 -> 珍藏与来华探访 ->
有些关系靠记住而不是等价还清”的故事成稿。运行为 `223.324s`、7 次调用、`36866 tokens`；正确性通过。该数据是首次孵化冷启动回执，包含新地图、搜索、选题和成稿，不得解读为后续每条内容的稳态成本。日常运营应复用已确认的内容根、版本化地图和 7 天 / 30 天计划，再从用户案例、公开热点、对标观察、新证据与历史回执中增量产生选题。最终回归为
